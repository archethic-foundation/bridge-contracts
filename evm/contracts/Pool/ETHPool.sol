...
contract ETHPool is PoolBase {
    ...
    function _createSignedHTLC(
        ...
        SignedHTLC_ETH htlcContract = (new SignedHTLC_ETH){value: _amount}(
            payable(msg.sender),
            _amount,
            _hash,
            _lockTime,
    ...
    function _createChargeableHTLC(
        ...
        ChargeableHTLC_ETH htlcContract = (new ChargeableHTLC_ETH){
            value: _amount
        }(
            _recipientAmount,
            _hash,
    ...
    function checkAmountWithDecimals(uint256 _amount) private pure {
    ...
    function getSwapsByOwner(
        address owner
    ) external view override returns (Swap[] memory swaps) {
    ...
    function setSwapByOwner(
        ...
        _swapsByOwner[_owner].push(
            Swap(_htlcContract, _archethicHTLCAddress, _swapType)
        );
    }
}
