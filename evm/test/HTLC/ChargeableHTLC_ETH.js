...
contract("Chargeable ETH HTLC", (accounts) => {
  ...
  it("withdraw should send tokens to the reserve address and fee to the safety module", async() => {
    ...
    assert.equal(balanceSafetyModuleAfter - balanceSafetyModuleBefore, web3.utils.toWei('0.005'))
    assert.equal(balanceReserveAfter - balanceReserveBefore, web3.utils.toWei('0.995'))
  })
