...
condition triggered_by: transaction, on: request_funds(end_time, amount, user_address, secret_hash, evm_tx_address, evm_contract, chain_id), as: [
  ...
  address: (
    ...
    if res.status == 200 && res.body != nil do
      ...
      valid? = valid_tx_receipt?(
        tx_receipt,
        chain_data.proxy_address,
        chain_data.token_address,
        evm_contract)
    end
...
fun valid_tx_receipt?(tx_receipt, proxy_address, token_address, evm_contract) do
  if tx_receipt != nil do
    logs = []
    if token_address == "NATIVE" do
      logs = List.at(tx_receipt.logs, 1)
    else
